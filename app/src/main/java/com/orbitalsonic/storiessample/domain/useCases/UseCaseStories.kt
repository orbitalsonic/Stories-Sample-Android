...
class UseCaseStories(
    ...
    private var currentStories = listOf<ItemStory>()
     ...
     */
    suspend fun getStoriesWithSeenStatus(): Result<Pair<List<ItemStory>, Map<Int, Boolean>>> = withContext(Dispatchers.Default) {
        try {
     ...
     */
    suspend fun refreshStories(): Result<Pair<List<ItemStory>, Map<Int, Boolean>>> = withContext(Dispatchers.IO) {
        try {
     ...
     */
    suspend fun getOriginalStories(): Result<List<ItemStory>> = withContext(Dispatchers.IO) {
        try {
     ...
     */
    fun getOriginalIndexForNavigation(clickedStory: ItemStory): Int {
     ...
     */
    private suspend fun getSeenStatusForStories(stories: List<ItemStory>): Map<Int, Boolean> {
    ...
    private fun sortStoriesBySeenStatus(
        stories: List<ItemStory>,
        seenStatusMap: Map<Int, Boolean>
    ): List<ItemStory> {
        return stories.sortedWith(compareBy<ItemStory> { story ->
     ...
     */
    suspend fun refreshSeenStatus(): Result<Pair<List<ItemStory>, Map<Int, Boolean>>> = withContext(Dispatchers.Default) {
        try {
